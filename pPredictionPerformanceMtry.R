...
perfData = foreach(i=1:nFiles, combine=data.frame,.packages = c('randomForest','readr','ROCR')) %dopar% {
  ...
  #mtry used (optimal mtry selected by tuneRF method)
  mtry = fit$mtry
  
  #return object
  data.frame(treeID = i, AUC = AUC, mtry = mtry, matches = matches, matchesPer100 = matchesPercentage, falsePositives = fp, falseNegatives = fn)
}
